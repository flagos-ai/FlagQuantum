...
| Layers, forbidden reverse dependencies, and decreasing Runtime-to-Compiler debt | `architecture.toml` | `tools/check_architecture.py` | `tests/unit/test_architecture_boundaries.py`, `.github/workflows/ci.yml` | Boundaries exist; machine-readable migration exit records remain to be added. |
...
| One source of truth for maturity and limitations | `capability-maturity.toml` | `tools/check_capability_maturity.py`, `tools/docs_source_of_truth.py --check` | `tests/unit/test_docs_source_of_truth.py`, `.github/workflows/ci.yml` | Existing; evidence-level schema awaits approval. |
