...
| Interoperability adapter contract | `experimental` | The adapter API is experimental and currently has one registered implementation, Qiskit. Registration does not install dependencies, sandbox third-party Python, certify provider hardware, or permit external objects to enter runtime and accelerator layers. |
