...
状态：历史盘点；当前实现已继续收敛，路径事实须以源码和机器台账为准
...
更新：2026-09-08 删除未发布的 RuntimePlan 0.9 迁移函数及专属测试，不保留兼容层；
当前 1.0 严格读取、序列化和哈希行为不变。本文后续出现的 `_compiler`、`compilation`
等路径记录的是盘点时状态，这些旧权威入口现已删除，不应作为新增实现依据。
