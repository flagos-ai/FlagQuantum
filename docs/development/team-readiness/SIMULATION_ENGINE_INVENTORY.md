...
| `simulation/jax_mps.py`、`simulation/jax_mps_batched.py`、`simulation/jax_mps_pullbacks.py` | 纯数值算法 | JAX MPS 单/双站点更新、批量 pair 分解、远程门 swap 路由、statevector 收缩、局部 observable、局部 VJP 与边界 pullback | 无 Runtime/Platform 依赖；Runtime 保留 circuit loop、shard 组织、参数所有权、通信和执行证据 |
