...
| `simulation/jax_gate_primitives.py`、`simulation/jax_statevector.py` | 纯数值算法 | JAX dtype、指令矩阵、statevector 本地执行、无通信门作用、跨 rank 门索引与 pair 合并数学、分片局部 observable/loss kernel | 无 Runtime/Platform 依赖；Runtime 保留 shard 组织、通信置换、pmap/shard-map、collective 和执行证据 |
