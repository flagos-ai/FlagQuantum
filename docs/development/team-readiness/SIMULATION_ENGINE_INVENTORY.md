...
| 分布式 MPS | `simulation/mps_rank_local.py`、`mps_site_kernels.py`、`mps_factorization.py` 及 reverse 尚未拆出的数值段 | `runtime/backends/mps/` 的 forward/state/distribution/communication/*transport/planning/training_engine/checkpointing/production/profiling | 前向本地指令分派、基础门作用、site kernel 和 QR 已归位；Runtime forward 仅决定所有权、通信、重平衡、生命周期和结果证据 |
