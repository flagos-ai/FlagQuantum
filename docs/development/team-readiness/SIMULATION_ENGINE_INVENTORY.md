...
| Split real/imag 与 Double-Single | `runtime/backends/statevector/split_real_imag*.py`；`simulation/double_single_device_gates.py` 的设备端门矩阵数值生成 | Runtime 文件中的平台身份、精度计划、conformance/result | 门矩阵数值 helper 已归 Simulation；实验路径不得成为首切片默认实现或被描述为等价 FP64 |
