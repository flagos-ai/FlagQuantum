...
| Split real/imag 与 Double-Single | `runtime/backends/statevector/split_real_imag*.py`；`simulation/double_single_host_gates.py` 与 `double_single_device_gates.py` 的隔离门矩阵生成；`simulation/double_single_statevector.py` 的门作用和归一化 | Runtime 文件中的平台身份、精度计划、conformance/result | P3/P4 门矩阵与共用纯数值 helper 已归 Simulation；主机/设备编码分文件保持 P4 来源审计；实验路径不得成为首切片默认实现或被描述为等价 FP64 |
