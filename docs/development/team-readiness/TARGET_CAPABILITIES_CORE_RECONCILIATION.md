...
| `runtime.platforms.PlatformDevice/Identity/MemorySnapshot` | CPU/CUDA/FlagOS platform provider | Runtime discovery、诊断 | 设备/运行时身份与可能未知的内存 | Identity 单向 `to_dict`；metadata 自由；无 snapshot hash | platform facts 输入；available 不等于 workload support |
