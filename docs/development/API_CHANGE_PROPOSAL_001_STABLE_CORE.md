...
**Proposal — 等待 API owner 批准。**

本文定义首次公开 alpha 之前的根 API 收敛方案。它不是已冻结契约，也不授权当前
提交直接删除或重命名接口。机器可读分类位于
...
- [ ] API owner 逐项批准 22 个 Stable Core 候选；
- [ ] 每个迁移接口的目标命名空间已存在或有明确实现任务；
- [ ] 仓库内没有依赖历史根路径的关键黄金用户路径；
- [ ] `ExecutionOptions` 和 `ExecutionPlan` 的后续提案已登记；
- [ ] CI 能证明 60 个当前导出全部且仅被分类一次；
- [ ] 明确首次公开 alpha 之前是否已有需要兼容的外部用户。

批准记录必须包含 reviewer、日期和目标版本。未满足这些条件时，本文件保持
`Proposal` 状态。
