#!/usr/bin/env python3
"""Validate checked-in selective Double-Single P2 A800 evidence."""
...
DEFAULT_ARTIFACT = ROOT / "artifacts/split_real_imag_precision_a800_20260824.json"
CONTRACT = ROOT / "contracts" / "split-real-imag-statevector-p2-precision-contract.toml"
...
def evidence_errors(payload: dict[str, Any]) -> tuple[str, ...]:
    ...
    if (
        payload.get("schema")
        != "flagquantum_split_real_imag_precision_a800_evidence_v1"
    ):
        errors.append("unexpected split P2 A800 evidence schema")
    if payload.get("status") != "passed":
        errors.append("split P2 A800 evidence must have passed status")
    ...
    if (
        ...
        errors.append("split P2 A800 evidence requires a clean full source revision")
    if len(str(source.get("archive_sha256", ""))) != 64:
        errors.append("split P2 A800 source archive hash is missing")
    ...
    if profile_payload != {"name": profile.name, "sha256": profile.profile_hash}:
        errors.append("split P2 A800 operator profile identity drifted")
    ...
    if payload.get("matrix") != {
        ...
        errors.append("split P2 A800 conformance matrix drifted")
    contract_thresholds = {
        name: value
    ...
    if payload.get("thresholds") != contract_thresholds:
        errors.append("split P2 A800 numerical thresholds drifted")
    if payload.get("conformance_workload") != contract["conformance_workload"]:
        errors.append("split P2 A800 cancellation workload drifted")
    ...
    if actual_cases != expected_cases or len(cases) != len(expected_cases):
        errors.append("split P2 A800 evidence case coverage is incomplete")
    for case in cases:
        if not case.get("passed", False):
            errors.append("split P2 A800 evidence contains a failed case")
        if float(case.get("expectation_abs_error", float("inf"))) > float(
            contract["thresholds"]["max_expectation_absolute_error"]
        ):
            errors.append("split P2 A800 expectation error exceeds contract")
        if float(case.get("gradient_relative_error", float("inf"))) > float(
            contract["thresholds"]["max_gradient_relative_error"]
        ):
            errors.append("split P2 A800 gradient error exceeds contract")
        if float(case.get("expectation_improvement_factor", 0.0)) < float(
            contract["thresholds"]["min_expectation_improvement_factor"]
        ):
            errors.append("split P2 A800 expectation improvement is below contract")
        if float(case.get("gradient_improvement_factor", 0.0)) < float(
            contract["thresholds"]["min_gradient_improvement_factor"]
        ):
            errors.append("split P2 A800 gradient improvement is below contract")
    ...
    if (
        ...
        errors.append("split P2 A800 native CUDA route is incomplete")
    if (
        ...
        errors.append("split P2 A800 Torch-FL flagos route is incomplete")

    claims = payload.get("claims", {})
    if claims.get("maturity") != "experimental":
        errors.append("split P2 A800 evidence must remain experimental")
    ...
    if any(claims.get(name) is not False for name in forbidden_true_claims):
        errors.append("split P2 A800 evidence contains a forbidden promotion claim")
    return tuple(errors)
...
def main() -> int:
    ...
    if errors:
        print("\n".join(errors))
        return 1
    print("Split real/imag P2 A800 evidence passed")
    return 0
