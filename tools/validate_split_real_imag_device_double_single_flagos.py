#!/usr/bin/env python3
"""Validate P4 device-generated Double-Single gates through Torch-FL flagos."""
...
def validate(
    *, device_name: str, depths: tuple[int, ...], seeds: tuple[int, ...]
    ...
    conformance = importlib.import_module(
        ...
        "split_real_imag_device_double_single_conformance"
    )
    ...
    report = conformance.run_split_real_imag_device_double_single_conformance(
        device, depths=depths, seeds=seeds
    )
    report.require_accepted()
    if report.device != str(device):
        raise AssertionError("P4 conformance escaped the logical flagos device")
    return {
        "schema": "flagquantum_split_real_imag_p4_flagos_reference_v1",
        "status": "passed",
