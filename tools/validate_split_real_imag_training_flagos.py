#!/usr/bin/env python3
"""Validate split FP32 P1 training through Torch-FL's logical FlagOS device."""
...
def validate(
    *, device_name: str, depths: tuple[int, ...], seeds: tuple[int, ...]
    ...
    report = split.run_split_real_imag_training_conformance(
        device, depths=depths, seeds=seeds
    )
    report.require_accepted()
    if report.device != str(device):
        raise AssertionError("P1 conformance escaped the logical flagos device")
    return {
        "schema": "flagquantum_split_real_imag_training_flagos_reference_v1",
        "status": "passed",
