...
class RuntimePolicy:
    """Serializable execution choice owned by ``QuantumModule``."""
    ...
    def __post_init__(self) -> None:
        if self.mode not in {
            ...
            raise ValueError(f"unsupported fq.QuantumModule mode {self.mode!r}")
        if self.backend not in {"pytorch", "jax"}:
            raise ValueError(f"unsupported fq.QuantumModule backend {self.backend!r}")
        if self.observable not in {"z", "z_sum", "hamiltonian"}:
            raise ValueError(
                f"unsupported fq.QuantumModule observable {self.observable!r}"
            )
