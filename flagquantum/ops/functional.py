...
def _recorded_wires(wires: List[int]) -> int | List[int]:
    return int(wires[0]) if len(wires) == 1 else [int(wire) for wire in wires]
...
def gate(
        ...
        q_device.op_history.append(
            {
                ...
                "wires": _recorded_wires(wires),
                "params": (
                    params.squeeze().detach().cpu().tolist()
                    if params is not None
                    else None
                ),
    ...
    assert matrix.shape[-1] == 2 ** len(wires)
    ...
    matrix = matrix.to(device=state.device, dtype=q_device.complex_dtype)
