...
from ..core._compilation_evidence_v2 import (
...
def _physical_evidence_v1(result: ArtifactCompilationResult) -> PhysicalPlanEvidence:
...
def _physical_evidence_v2(result: ArtifactCompilationResult) -> PhysicalPlanEvidenceV2:
    plan = result.physical_plan
    if (
        ...
        raise CompilationEvidenceError(
            "compilation evidence version 2.0 requires a complete directed plan"
        )
...
def build_compilation_evidence_bundle(
    ...
    *,
    ...
    source = {
        ...
        "source_circuit_hash": result.physical_plan.source_circuit_hash,
    ...
    if result.physical_plan.coupling_direction_semantics == "directed_cx":
        return CompilationEvidenceBundleV2(
            ...
            source=source,
            target=target,
            physical_plan=_physical_evidence_v2(result),
            output=output,
        )
    return CompilationEvidenceBundle(
        ...
        source=source,
        target=target,
        physical_plan=_physical_evidence_v1(result),
        output=output,
    )
