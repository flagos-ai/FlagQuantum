...
class QuafuProvider(HttpQuantumProvider):
    ...
    def fetch_chip_info(self, chip: str) -> Mapping[str, Any]:
        ...
        This is the same platform endpoint used by
        ``quark.circuit.Backend(chip).chip_info``.  The returned mapping is
        intentionally preserved verbatim so callers can archive immutable
        experiment provenance before submission.
        """
    ...
    def submit_physical_qasm(
        ...
        *,
        ...
        This path is intended for experiments that must seal the exact physical
        circuit before execution.  The program digest is propagated through
        the normal Quafu result identity chain.
        """
