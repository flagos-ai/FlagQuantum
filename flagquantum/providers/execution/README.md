...
`braket.py`, `cqlib.py`, `fieldquantum.py`, `local.py`, `originq.py`, `quafu.py`,
and `tencent.py`; shared HTTP transport and response normalization live in
`http.py` and `result_parsing.py`. Quafu calibration conversion lives beside
its adapter in `quafu_calibration.py`. Run `python -m pytest
