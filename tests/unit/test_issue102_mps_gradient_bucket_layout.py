...
from flagquantum.runtime.executors.mps.reverse_planning import plan_mps_gradient_buckets
...
def test_thousand_parameters_collapse_to_owner_dtype_buckets():
    ...
    buckets = plan_mps_gradient_buckets(parameters, owners, max_bucket_bytes=4096)
...
def test_bucket_layout_splits_large_parameters_and_separates_dtype():
    ...
    buckets = plan_mps_gradient_buckets(parameters, (0, 0), max_bucket_bytes=16)
    assert all(
        sum(
    ...
    with pytest.raises(ValueError, match="match"):
        plan_mps_gradient_buckets(parameters, (0,), max_bucket_bytes=16)
    with pytest.raises(ValueError, match="positive"):
        plan_mps_gradient_buckets(parameters, (0, 0), max_bucket_bytes=0)
