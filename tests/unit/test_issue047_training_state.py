...
def test_checkpoint_rejects_non_equivalent_topology(tmp_path, monkeypatch) -> None:
    ...
    seed = fq.seed_everything(7)
    source = fq.Module(build, 2)
    source.execute()
    path = source.save_checkpoint(tmp_path / "topology.pt", optimizer=None, seed=seed)
    target = fq.Module(build, 2)
    ...
    with pytest.raises(fq.TopologyMismatchError):
...
def test_checkpoint_rejects_different_workload_and_optimizer_presence(tmp_path) -> None:
    ...
    with pytest.raises(fq.TrainingStateError, match="optimizer type"):
...
def test_checkpoint_signature_preserves_fixed_tensor_gate_values(tmp_path) -> None:
    ...
    source.execute()
    path = source.save_checkpoint(
        tmp_path / "fixed.pt", optimizer=None, seed=fq.seed_everything(17)
    )
    with pytest.raises(fq.TrainingStateError, match="workload"):
...
def test_public_loader_installs_saved_precision_policy(tmp_path) -> None:
    ...
    path = fq.save_training_checkpoint(
        ...
        module=source,
        optimizer=None,
        seed=fq.seed_everything(13),
        precision=policy,
    )
    target = fq.Module(build, 2)
    restored = fq.load_training_checkpoint(
        path, module=target, optimizer=None, precision=None
    )
...
def test_precision_policy_is_explicit_and_forbids_silent_downcast() -> None:
    ...
    with pytest.raises(fq.PrecisionPolicyError, match="downcast"):
        fq.PrecisionPolicy(parameter_dtype="float32").apply(high)
    policy = fq.PrecisionPolicy(
...
def test_seed_streams_and_correctness_debug_are_reproducible() -> None:
    ...
    with pytest.raises(fq.NonFiniteTrainingError, match="parameter"):
        debug.execute()
...
def test_checkpoint_restore_installs_correctness_debug_gradient_hook(tmp_path) -> None:
    ...
    source.execute()
    path = source.save_checkpoint(
        tmp_path / "debug.pt", optimizer=None, seed=fq.seed_everything(31)
    )
    target = fq.Module(build, 2)
    target.load_checkpoint(path, optimizer=None)
    assert target.policy.correctness_debug is True
    assert target._correctness_debug_hook_handle is not None
    with pytest.raises(fq.NonFiniteTrainingError, match="gradient"):
