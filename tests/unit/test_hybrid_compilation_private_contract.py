...
def test_phase27_adds_bounded_topology_legality_without_target_ir() -> None:
    ...
    assert contract["status"] == ("phase41_directional_evidence_handoff_complete")
...
def test_phase32_implements_only_the_approved_core_artifact_scope() -> None:
    contract = _contract()
    ...
    assert phase32["approval_date"] == "2026-09-10"
    assert phase32["implementation"] == (
        "flagquantum.core._artifacts.ProgramArtifactV2"
    )
    assert phase32["explicit_version_dispatch"] is True
    assert phase32["v1_reader_unchanged"] is True
    assert phase32["v1_content_hash_unchanged"] is True
    assert phase32["circuit_profile_constructor"] is True
    assert phase32["executable_profile_validation"] is True
    assert phase32["bounded_v1_circuit_migration"] is True
    assert phase32["duplicate_json_keys_rejected"] is True
    assert phase32["closed_limits_enforced"] is True
    assert phase32["compiler_adapter_added"] is False
    assert phase32["runtime_adapter_added"] is False
    assert phase32["deployment_adapter_added"] is False
    assert phase32["public_root_export"] is False
    assert phase32["default_path_change"] is False
    assert phase32["performance_claim"] is False
    assert phase32["evidence_report"] == (
        "docs/development/HYBRID_COMPILATION_PHASE32_EVIDENCE.md"
    )
    assert contract["phase32_completed"] is True


def test_phase33_connects_artifacts_without_reinterpreting_target_text() -> None:
    contract = _contract()
    ...
    assert phase33["runtime_target_preflight"] == (
        "flagquantum.runtime.artifact_preflight.preflight_executable_artifact"
    )
    assert phase33["deployment_dry_run"] == (
        "flagquantum.deployment.artifact_dry_run.prepare_artifact_deployment"
    )
    assert phase33["runtime_local_circuit_execution"] == (
        "flagquantum.runtime.artifact_execution.execute_circuit_artifact"
    )
    assert phase33["accepted_local_profile"] == "circuit-ir-1.0_fully_bound"
    assert phase33["target_text_local_simulation"] is False
    assert phase33["symbolic_artifact_execution"] is False
    assert phase33["request_fields_change_artifact_identity"] is False
    assert phase33["result_records_artifact_identity"] is True
    assert phase33["provider_submission_added"] is False
    assert phase33["public_root_export"] is False
    assert phase33["default_path_change"] is False
    assert phase33["performance_claim"] is False
    assert phase33["evidence_report"] == (
...
def test_phase35_makes_the_complete_artifact_compilation_chain_mandatory() -> None:
    contract = _contract()
    ...
    assert phase35["result"] == (
        "flagquantum.compiler.artifact_compilation.ArtifactCompilationResult"
    )
    assert phase35["accepted_inputs"] == [
        "fully_bound_circuit-ir-1.0",
        "CircuitArtifactBindingResult",
    ]
    assert phase35["ordered_stages"] == [
        "circuit_artifact_identity_verification",
        "target_capability_legalization",
        "topology_routing_when_requested",
        "native_gate_legalization",
        "logical_scheduling",
        "deterministic_target_emission",
        "strict_target_conformance",
        "executable_artifact_construction",
    ]
    assert phase35["input_binding_lineage_preserved"] is True
    assert phase35["compilation_identity_is_deterministic"] is True
    assert phase35["stages_bypassable"] is False
    assert phase35["compiler_numerical_execution"] is False
    assert phase35["provider_submission_added"] is False
    assert phase35["public_root_export"] is False
    assert phase35["default_path_change"] is False
    assert phase35["performance_claim"] is False
    assert phase35["evidence_report"] == (
        "docs/development/HYBRID_COMPILATION_PHASE35_EVIDENCE.md"
    )
    assert contract["phase35_completed"] is True


def test_phase36_composes_physical_evidence_without_a_second_ir() -> None:
    contract = _contract()
    ...
    assert phase36["result"] == (
        "flagquantum.compiler.physical_plan.PhysicalCircuitPlan"
    )
    assert phase36["authoritative_program"] == "flagquantum.core.ir.CircuitIR"
    assert phase36["source_program_objects_retained"] is True
    assert phase36["mapping_transitions_replayable"] is True
    assert (
        phase36["physical_instructions_link_source_topology_native_and_schedule"]
        is True
    )
    assert phase36["coupling_edges_verified"] is True
    assert phase36["critical_path_is_dependency_based"] is True
    assert phase36["identity_is_deterministic"] is True
    assert phase36["artifact_compilation_requires_plan"] is True
    assert phase36["target_ir_added"] is False
    assert phase36["timing_or_duration_claim"] is False
    assert phase36["pulse_or_calibration_claim"] is False
    assert phase36["physical_ancilla_allocation"] is False
    assert phase36["public_root_export"] is False
    assert phase36["default_path_change"] is False
    assert phase36["performance_claim"] is False
    assert phase36["evidence_report"] == (
        "docs/development/HYBRID_COMPILATION_PHASE36_EVIDENCE.md"
    )
    assert contract["phase36_completed"] is True


def test_phase37_completes_approved_evidence_handoff() -> None:
    contract = _contract()
    ...
    assert phase37["core_implementation_authorized"] is True
    assert phase37["compiler_builder_authorized"] is True
    assert phase37["runtime_adapter_authorized"] is True
    assert phase37["deployment_adapter_authorized"] is True
    ...
    assert contract["phase37_core_compiler_completed"] is True
    assert contract["phase37_runtime_deployment_completed"] is True
...
def test_phase38_records_approved_directional_topology_contract() -> None:
    ...
    assert phase38["candidate_contract"] == (
        "contracts/directional-topology-layout-v2-candidate.json"
    )
    assert phase38["approval_token"] == (
        "approve API_CHANGE_PROPOSAL_024_DIRECTIONAL_TOPOLOGY_LAYOUT"
    )
    assert phase38["candidate_evidence_version"] == "2.0"
    assert phase38["coupling_map_changed"] is False
    assert phase38["compilation_evidence_v1_changed"] is False
    ...
    assert phase38["evidence_report"] == (
        "docs/development/HYBRID_COMPILATION_PHASE38_EVIDENCE.md"
    )
    assert contract["phase38_proposal_completed"] is True
    assert contract["phase38_implementation_completed"] is True
...
def test_phase40_completes_directional_evidence_core_compiler_slice() -> None:
    contract = _contract()
    phase40 = contract["phase40"]

    assert contract["status"] == "phase41_directional_evidence_handoff_complete"
    assert phase40["core_bundle"] == (
        "flagquantum.core._compilation_evidence_v2.CompilationEvidenceBundleV2"
    )
    assert phase40["core_plan"] == (
        "flagquantum.core._compilation_evidence_v2.PhysicalPlanEvidenceV2"
    )
    assert phase40["core_coupling"] == (
        "flagquantum.core._compilation_evidence_v2.DirectedCouplingEvidence"
    )
    assert phase40["explicit_version_dispatch"] is True
    assert phase40["v1_reader_and_identity_unchanged"] is True
    assert phase40["closed_records_and_limits"] is True
    assert phase40["direction_groups_structurally_verified"] is True
    assert phase40["compiler_builds_from_actual_plan"] is True
    assert phase40["compiler_verifies_against_actual_compilation"] is True
    assert phase40["runtime_adapter_added"] is False
    assert phase40["deployment_adapter_added"] is False
    assert phase40["public_root_export"] is False
    assert phase40["default_path_change"] is False
    assert phase40["evidence_report"] == (
        "docs/development/HYBRID_COMPILATION_PHASE40_EVIDENCE.md"
    )
    assert contract["phase40_core_compiler_completed"] is True


def test_phase41_completes_directional_runtime_deployment_handoff() -> None:
    contract = _contract()
    phase41 = contract["phase41"]

    assert contract["status"] == "phase41_directional_evidence_handoff_complete"
    assert phase41["runtime_verifier"] == (
        "flagquantum.runtime.compilation_evidence."
        "verify_compilation_evidence_handoff"
    )
    assert phase41["deployment_dry_run"] == (
        "flagquantum.deployment.artifact_dry_run.prepare_artifact_deployment"
    )
    assert phase41["actual_source_required"] is True
    assert phase41["actual_target_snapshot_required"] is True
    assert phase41["actual_executable_artifact_required"] is True
    assert phase41["evidence_rewritten"] is False
    assert phase41["provider_submission_added"] is False
    assert phase41["credentials_or_job_state_added"] is False
    assert phase41["program_artifact_v1_changed"] is False
    assert phase41["program_artifact_v2_changed"] is False
    assert phase41["public_root_export"] is False
    assert phase41["default_path_change"] is False
    assert phase41["evidence_report"] == (
        "docs/development/HYBRID_COMPILATION_PHASE41_EVIDENCE.md"
    )
    assert contract["phase41_runtime_deployment_completed"] is True
    assert contract["phase38_implementation_completed"] is True
