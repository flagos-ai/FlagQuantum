...
class TestTwoQubitGates:
    ...
    def test_bell_state(self, device):
        ...
        # In Bell state, individual expectations are 0
        result = fq.measure_allZ(device)
        assert torch.abs(result[0, 0]) < 0.1
        assert torch.abs(result[0, 1]) < 0.1

        # Check correlation: both qubits should have same sign
        # (both positive or both negative)
        sign_product = result[0, 0] * result[0, 1]
        assert sign_product >= 0  # Same sign indicates correlation
...
class TestParameterizedGates:
    ...
    def test_parameter_gradient(self, device):
        ...
        # Create trainable parameter (1D tensor for proper gradients)
        theta = torch.tensor([0.5], requires_grad=True)
        device.rx(wires=[0], params=theta)

        # Measure and take sum to get scalar
        result = fq.measure_allZ(device)  # shape (1, 1)
        loss = result.abs().sum()  # scalar (0-dim tensor)
    ...
    def test_inverse_gate(self, device):
        ...
        # Apply inverse using negative parameter (rx_inv may not exist)
        device.rx(wires=[0], params=-theta)

        # Should return to initial |0> state (Z expectation = 1)
        result = fq.measure_allZ(device)
        assert torch.abs(result[0, 0] - 1.0) < 1e-5
...
class TestGateCommutations:
    ...
    def test_x_and_z_anticommute(self, device):
        ...
        # Both sequences should give |1> state
