...
CANDIDATE = ROOT / "contracts/ir-phase3-batch-a-exit-batch-b-review-candidate.json"
...
def test_batch_a_exit_evidence_is_complete_and_has_no_known_blocker() -> None:
    evidence = _candidate()["batch_a_exit_evidence"]

    assert evidence["authorized_scope_implemented"] is True
    assert evidence["closed_schema_and_fail_closed_validation"] is True
    assert evidence["canonical_identity_and_hash_seed_determinism"] is True
    assert evidence["mutation_isolation"] is True
    assert evidence["structured_compatibility_diagnostics"] is True
...
def test_batch_b_candidate_scope_is_private_and_narrow() -> None:
    ...
    assert scope["feature"] == "private TargetIR and deterministic legalization"
    assert scope["executable_artifact_authorized"] is False
    assert scope["runtime_adapter_authorized"] is False
    ...
    assert scope["batch_c_through_h_authorized"] is False
    ...
    assert candidate["approval_command"] == ("approve IR-PHASE3-BATCH-A-EXIT-BATCH-B")
