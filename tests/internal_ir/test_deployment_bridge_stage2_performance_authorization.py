...
VALIDATION = (
    ROOT / "contracts/deployment-bridge-stage2-performance-budget-validation.json"
)
...
def test_stage2_performance_authorization_binds_review_budget_and_baseline() -> None:
...
def test_stage2_performance_authorization_keeps_later_work_closed() -> None:
    ...
    assert decisions["stage3_proposal_authorized"] is False
...
def test_stage2_budget_validation_is_bound_and_fail_closed() -> None:
    ...
    assert validation["decisions"]["stage2_exit_authorized"] is False
    assert validation["decisions"]["stage3_proposal_authorized"] is False
