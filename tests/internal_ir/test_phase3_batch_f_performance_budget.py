...
from benchmarks.internal import ir_phase3_batch_f_gate as gate
...
def test_batch_f_machine_gate_accepts_boundary_and_rejects_regression(
    ...
    budget_by_size = {item["comparison_count"]: item for item in budgets}
    ...
    def observation(
        ...
        budget = budget_by_size[comparison_count]
        latency = budget["match_shadow_p95_ms_max"]
        if regression["enabled"] and comparison_count == 1000:
            latency += 0.001
        return {
            "comparison_count": comparison_count,
            "match_shadow_p95_ms": latency,
            "match_shadow_peak_host_memory_bytes": budget[
                "match_shadow_peak_host_memory_bytes_max"
            ],
    ...
    assert failed["comparison_count"] == 1000
