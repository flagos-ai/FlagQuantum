...
AUTHORIZATION = (
    ROOT / "contracts/ir-phase3-batch-c-performance-budget-authorization.json"
)
VALIDATION = ROOT / "contracts/ir-phase3-batch-c-performance-budget-validation.json"
...
def test_batch_c_performance_authorization_binds_review_budget_and_baseline() -> None:
...
def test_batch_c_performance_authorization_keeps_later_work_closed() -> None:
    ...
    assert decisions["batch_c_exit_authorized"] is False
    assert decisions["batch_d_runtime_adapter_authorized"] is False
...
def test_batch_c_budget_validation_is_bound_and_fail_closed() -> None:
    ...
    assert validation["decisions"]["batch_c_exit_authorized"] is False
    assert validation["decisions"]["batch_d_runtime_adapter_authorized"] is False
