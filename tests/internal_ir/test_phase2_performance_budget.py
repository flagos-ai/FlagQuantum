...
BUDGET = ROOT / "tests/fixtures/internal_ir/phase2_performance_budget_candidate.json"
...
def test_phase2_budget_is_approved_only_through_bound_authorization() -> None:
    budget = json.loads(BUDGET.read_text(encoding="utf-8"))
    authorization = json.loads(AUTHORIZATION.read_text(encoding="utf-8"))

    assert budget["status"] == "pending_owner_approval"
    assert authorization["decisions"]["phase2_performance_budget_approved"] is True
    assert authorization["approved_budget"] == str(BUDGET.relative_to(ROOT))
    assert (
...
@pytest.mark.xfail(
    strict=True,
    reason="IR2A-PERF-001: approved 1K pipeline budget is currently exceeded",
)
def test_approved_phase2_batch_a_budget_is_machine_enforced() -> None:
    budget = json.loads(BUDGET.read_text(encoding="utf-8"))
