...
BASELINE = ROOT / "tests/fixtures/internal_ir/phase3_batch_c_performance_baseline.json"
BUDGET = ROOT / "tests/fixtures/internal_ir/phase3_batch_c_performance_budget.json"
...
def test_batch_c_performance_profile_is_internal() -> None:
...
def test_batch_c_budget_envelopes_cover_each_observed_baseline() -> None:
    baseline = _load(BASELINE)
    ...
    for count, case in observed.items():
        ...
        assert case["seal_and_verify_p95_ms"] <= budget["seal_and_verify_p95_ms_max"]
        assert (
