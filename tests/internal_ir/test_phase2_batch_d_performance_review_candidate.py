...
CANDIDATE = (
    ROOT / "contracts/ir-phase2-batch-d-performance-budget-review-candidate.json"
)
SUCCESSOR = (
    ROOT / "contracts/ir-phase2-batch-d-performance-budget-artifact-successor.json"
)
...
def test_batch_d_performance_review_binds_authorization_and_artifacts() -> None:
...
def test_batch_d_gate_successor_is_authorized_and_scope_closed() -> None:
    successor = json.loads(SUCCESSOR.read_text(encoding="utf-8"))
...
def test_batch_d_budget_review_keeps_exit_and_later_scope_closed() -> None:
    candidate = json.loads(CANDIDATE.read_text(encoding="utf-8"))
