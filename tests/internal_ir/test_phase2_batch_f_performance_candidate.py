...
BASELINE = ROOT / "contracts/ir-phase2-batch-f-performance-baseline.json"
BUDGET = (
    ROOT / "tests/fixtures/internal_ir/phase2_batch_f_performance_budget_candidate.json"
)
...
def test_batch_f_budget_is_unapproved_and_above_observed_baseline() -> None:
    baseline = json.loads(BASELINE.read_text(encoding="utf-8"))
    budget = json.loads(BUDGET.read_text(encoding="utf-8"))
    measured = {case["gate_count"]: case for case in baseline["cases"]}

    assert budget["status"] == "candidate_awaiting_owner_approval"
    assert budget["approval"]["approved"] is False
    assert budget["approval"]["approval_command"] == (
