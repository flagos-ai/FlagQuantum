...
CANDIDATE = ROOT / "contracts/ir-phase3-batch-c-exit-batch-d-review-candidate.json"
...
def test_batch_c_exit_candidate_binds_authorizations_and_artifacts() -> None:
...
def test_batch_c_exit_evidence_is_complete_and_has_no_known_blocker() -> None:
    evidence = _candidate()["batch_c_exit_evidence"]

    assert evidence["authorized_scope_implemented"] is True
    assert evidence["profile_payload_and_identity_chain"] is True
    assert evidence["tamper_and_mismatch_fail_closed"] is True
    assert evidence["privacy_boundary_enforced"] is True
    assert evidence["deterministic_anonymous_fixtures"] is True
...
def test_batch_d_candidate_scope_is_private_and_narrow() -> None:
    candidate = _candidate()
    scope = candidate["batch_d_scope_if_approved"]
    decisions = candidate["proposed_decisions"]

    assert scope["feature"] == (
        "private RuntimeAdapter protocol and execution-binding contract"
    )
    assert scope["provider_sdk_or_remote_submission_authorized"] is False
    assert scope["credentials_authorized"] is False
    assert scope["real_backend_identity_in_anonymous_evidence_authorized"] is False
    ...
    assert scope["batch_e_through_h_authorized"] is False
    ...
    assert candidate["approval_command"] == ("approve IR-PHASE3-BATCH-C-EXIT-BATCH-D")
