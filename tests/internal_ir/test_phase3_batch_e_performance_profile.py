...
BASELINE = ROOT / "tests/fixtures/internal_ir/phase3_batch_e_performance_baseline.json"
BUDGET = ROOT / "tests/fixtures/internal_ir/phase3_batch_e_performance_budget.json"
...
def test_batch_e_performance_profile_is_internal() -> None:
...
def test_batch_e_budget_envelopes_cover_each_observed_baseline() -> None:
    baseline = _load(BASELINE)
    budget_record = _load(BUDGET)
    observed = {item["suite_count"]: item for item in baseline["cases"]}
    budgets = {item["suite_count"]: item for item in budget_record["budgets"]}
