...
VALIDATION = (
    ROOT / "contracts/deployment-bridge-stage3-performance-budget-validation.json"
)
...
def test_stage3_performance_authorization_binds_review_budget_and_baseline() -> None:
...
def test_stage3_performance_authorization_keeps_later_work_closed() -> None:
    decisions = json.loads(AUTHORIZATION.read_text(encoding="utf-8"))["decisions"]

    allowed = {
        "stage3_performance_budget_approved",
        "stage3_machine_gate_authorized",
    }
    assert all(decisions[name] is True for name in allowed)
    assert decisions["stage3_exit_authorized"] is False
    assert decisions["stage4_proposal_authorized"] is False
    assert all(
        value is False for name, value in decisions.items() if name not in allowed
    )


def test_stage3_budget_validation_is_bound_and_fail_closed() -> None:
    ...
    assert validation["decisions"]["stage3_exit_authorized"] is False
    assert validation["decisions"]["stage4_proposal_authorized"] is False
