...
CANDIDATE = ROOT / "contracts/ir-phase2-batch-d-exit-batch-e-review-candidate.json"
...
def test_batch_d_exit_candidate_binds_authorization_validation_and_artifacts() -> None:
...
def test_batch_e_candidate_keeps_later_scope_closed() -> None:
    candidate = json.loads(CANDIDATE.read_text(encoding="utf-8"))
    decisions = candidate["proposed_decisions"]

    assert decisions["batch_d_exit_approved"] is True
    assert decisions["batch_e_private_pipeline_infrastructure_authorized"] is True
    assert decisions["batch_e_performance_baseline_authorized"] is True
    ...
    assert decisions["persistent_or_distributed_cache_authorized"] is False
    ...
    assert candidate["approval_command"] == "approve IR-PHASE2-BATCH-D-EXIT-BATCH-E"
