...
BASELINE = ROOT / "contracts/ir-phase2-batch-c-performance-baseline.json"
BUDGET = (
    ROOT / "tests/fixtures/internal_ir/phase2_batch_c_performance_budget_candidate.json"
)


def test_batch_c_budget_candidate_is_measured_unapproved_and_internal() -> None:
...
def test_batch_c_candidate_cannot_authorize_exit_or_later_work() -> None:
    budget = json.loads(BUDGET.read_text(encoding="utf-8"))

    assert budget["later_scope"]["batch_c_exit_authorized"] is False
    assert budget["later_scope"]["batch_d_authorized"] is False
