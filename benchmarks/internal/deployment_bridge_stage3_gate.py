"""Enforce the approved private Deployment Bridge Stage 3 budget."""
...
from benchmarks.internal.deployment_bridge_stage3_baseline import measure_case
...
DEFAULT_BUDGET = (
    ...
    / "fixtures"
    / "internal_ir"
    / "deployment_bridge_stage3_performance_budget_candidate.json"
)
DEFAULT_AUTHORIZATION = (
    ROOT / "contracts/deployment-bridge-stage3-performance-budget-authorization.json"
)
APPROVAL_COMMAND = "approve DEPLOYMENT-BRIDGE-STAGE3-PERFORMANCE-BUDGET"
...
def evaluate(
    ...
    *,
    ...
    if authorization.get("status") != "approved":
        raise ValueError("Stage 3 performance budget is not approved")
    if authorization.get("approval_command") != APPROVAL_COMMAND:
        raise ValueError("unexpected Stage 3 budget authorization command")
    approved_budget = authorization.get("approved_budget", {})
    if approved_budget.get("sha256") != _sha256(budget_path):
        raise ValueError("Stage 3 budget does not match its authorization")
    if budget_payload["approval"]["approval_command"] != APPROVAL_COMMAND:
        raise ValueError("unexpected Stage 3 budget approval command")
    budgets = {
        int(item["evaluation_count"]): item for item in budget_payload["budgets"]
    }
    if set(budgets) != {10, 100, 1000, 10000}:
        raise ValueError("Stage 3 budget must cover 10/100/1K/10K evaluations")
    cases = []
    for evaluation_count in (10, 100, 1000, 10000):
        case = measure_case(
            evaluation_count,
            iterations=iterations,
            warmup=warmup,
        )
        budget = budgets[evaluation_count]
        case["budget"] = budget
        case["latency_passed"] = (
    ...
    return {
        ...
        "claim_scope": "Deployment Bridge Stage 3 private CPU budget; not a public SLA",
