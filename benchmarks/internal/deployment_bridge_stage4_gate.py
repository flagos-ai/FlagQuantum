...
from benchmarks.internal.deployment_bridge_stage4_baseline import measure_case
...
def evaluate(
    ...
    *,
    ...
    cases = []
    for rehearsal_count in (10, 100, 1000, 10000):
        case = measure_case(
            rehearsal_count,
            iterations=iterations,
            warmup=warmup,
        )
        budget = budgets[rehearsal_count]
        case["budget"] = budget
        case["latency_passed"] = (
            case["offline_rehearsal_p95_ms"] <= budget["offline_rehearsal_p95_ms_max"]
        )
        case["memory_passed"] = (
