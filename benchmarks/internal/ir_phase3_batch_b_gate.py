"""Enforce the approved private Phase 3 Batch B performance budget."""
...
from benchmarks.internal.ir_phase3_batch_b_baseline import measure_case
...
DEFAULT_BUDGET = (
    ...
    / "fixtures"
    / "internal_ir"
    / "phase3_batch_b_performance_budget_candidate.json"
)
...
def evaluate(budget_path: Path, *, iterations: int, warmup: int) -> dict[str, Any]:
    ...
    if payload["approval"]["approval_command"] != (
        ...
        raise ValueError("unexpected Batch B budget approval command")
    budgets = {int(item["operation_count"]): item for item in payload["budgets"]}
    if set(budgets) != {10, 100, 1000, 10000}:
        raise ValueError("Batch B budget must cover 10/100/1K/10K operations")
    cases = []
    for operation_count in (10, 100, 1000, 10000):
        ...
        case["budget"] = budget
        case["latency_passed"] = (
            case["legalization_p95_ms"] <= budget["legalization_p95_ms_max"]
        )
        case["memory_passed"] = (
            case["legalization_peak_host_memory_bytes"]
    ...
    return {
        ...
        "claim_scope": "Phase 3 Batch B private CPU budget; not a public SLA",
