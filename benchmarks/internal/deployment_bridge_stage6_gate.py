"""Enforce the approved private Deployment Bridge Stage 6 budget."""
...
DEFAULT_BUDGET = (
    ...
    / "fixtures"
    / "internal_ir"
    / "deployment_bridge_stage6_performance_budget_candidate.json"
)
DEFAULT_AUTHORIZATION = (
    ROOT / "contracts/deployment-bridge-stage6-performance-budget-authorization.json"
)
APPROVAL_COMMAND = "approve DEPLOYMENT-BRIDGE-STAGE6-PERFORMANCE-BUDGET"
...
def evaluate(
    ...
    *,
    ...
    if authorization.get("status") != "approved":
        raise ValueError("Stage 6 performance budget is not approved")
    if authorization.get("approval_command") != APPROVAL_COMMAND:
        raise ValueError("unexpected Stage 6 budget authorization command")
    approved_budget = authorization.get("approved_budget", {})
    if approved_budget.get("sha256") != _sha256(budget_path):
        raise ValueError("Stage 6 budget does not match its authorization")
    if budget_payload["approval"]["approval_command"] != APPROVAL_COMMAND:
        raise ValueError("unexpected Stage 6 budget approval command")
