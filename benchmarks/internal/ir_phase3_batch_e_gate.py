"""Enforce the approved private Phase 3 Batch E performance budget."""
...
from benchmarks.internal.ir_phase3_batch_e_baseline import measure_case
...
DEFAULT_BUDGET = (
    ...
    / "fixtures"
    / "internal_ir"
    / "phase3_batch_e_performance_budget_candidate.json"
)
...
def evaluate(budget_path: Path, *, iterations: int, warmup: int) -> dict[str, Any]:
    ...
    if payload["approval"]["approval_command"] != (
        ...
        raise ValueError("unexpected Batch E budget approval command")
    budgets = {int(item["suite_count"]): item for item in payload["budgets"]}
    if set(budgets) != {10, 100, 1000, 10000}:
        raise ValueError("Batch E budget must cover 10/100/1K/10K suites")
    cases = []
    for suite_count in (10, 100, 1000, 10000):
        case = measure_case(
            suite_count,
            iterations=iterations,
            warmup=warmup,
        )
        budget = budgets[suite_count]
        case["budget"] = budget
        case["latency_passed"] = (
    ...
    return {
        ...
        "claim_scope": "Phase 3 Batch E private CPU budget; not a public SLA",
